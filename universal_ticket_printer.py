...
def _warmup_manifest():
    ...
    for package in packages:
        try:
            _run_miktex_command(["mpm", "--admin", "--install", package], timeout=180)
        except Exception as exc:
            _log_debug(f"Manifest warm-up failed for {package}: {exc}")
    try:
        _run_miktex_command(["initexmf", "--admin", "--update-fndb"], timeout=180)
    except Exception as exc:
...
def _run_miktex_command(
    cmd: List[str],
    cwd: Optional[str] = None,
    timeout: int = 10
) -> Tuple[int, str, str]:
    with MIKTEX_LOCK:
        return _run_subprocess_logged(cmd, cwd=cwd, timeout=timeout)
...
def _check_pdflatex():
    try:
        _run_miktex_command(["pdflatex", "--version"], timeout=5)
        return True
...
""" % content
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                exit_code, stdout, stderr = _run_miktex_command(cmd, cwd=temp_dir, timeout=compile_timeout)
                if exit_code != 0:
                ...
                if os.path.exists(log_path):
                    ...
                    if missing_dep:
                        ...
                        try:
                            ...
                            _run_miktex_command(install_args, timeout=180)
